...
(`adaptationCoverage` in `src/lib/adaptations.ts`):
...
(`src/lib/db/adaptationTargets.ts`), and
...
(`classifyCardioByDuration` in `src/lib/adaptations.ts`).
