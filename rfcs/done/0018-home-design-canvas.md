---
title: Home design canvas — a JIT design system for the fused read
authors: [Peter Petrov]
created: 2026-08-27
last_updated: 2026-09-10
status: done
status_note: "rounds 1–3 settled the design, both five-second tests passed, and step 8 shipped it in six units (v1.1.6 constants + fused-read library; v1.2.0 SIGNAL tokens + the T1 surface; v1.2.1 + v1.3.0 the T2 sheets; v1.4.0 the three folds, the recovery sheet and R1's cap met; v1.4.1 the T3 split, T1 halved to 554 kB / 161 kB gzip; v1.4.2 the regression pass and `show_in_home`). One piece of it is deliberately parked, not dropped: the `show_in_home` **column** cannot go until 2.0.0 reaches master, and waits in [0025-release-blocked-schema-drops.md](0025-release-blocked-schema-drops.md)."
label: feature
---

# RFC 0018: Home design canvas — a JIT design system for the fused read

**Canvas URL:** https://claude.ai/code/artifact/a1534123-0c92-49dc-8fa1-3879279d16ee Rounds 2 and 3 **update that same canvas** — they never invoke `/design` for a fresh one, or the user's own edits in the editor are lost.
**Working files:** `design/home-canvas/` — committed to the repo, not the scratchpad. The canvas is re-seeded from these files on every change, so they have to outlive the session that made them. They sit outside `src/`, so they add nothing to the bundle.
**Origin:** [0010-home-fused-reads.md](0010-home-fused-reads.md) §6 says its two open questions are "easier to answer with the fused state concrete than in the abstract." That is a request for a mockup. This brief is that mockup, plus the design system it needs.
**Blocks:** the three remaining folds in [0014-doctrine-ledger-execution.md](0014-doctrine-ledger-execution.md) and, through them, R1's cap of four menu sections.
---
...
- **2026-08-29 — five-second test #1 passed**, performed by Peter on the fused
  ...
  [0010-home-fused-reads.md](0010-home-fused-reads.md) §6 — the gate changes the
...
- **2026-08-30 — step 7 done:** `/ground` ran; five scout blocks landed in
  [010 §Grounding](0010-home-fused-reads.md#grounding), verdicts recorded in the
  grounding inventory.
...
- **2026-08-31 — step 8 unit 4 shipped** (v1.4.0): the three folds, and R1's cap
  ...
  **HOME / WEIGHTS / CARDIO / PROGRAM / MORE** (Adaptations and Mobility move
  ...
  [0014](0014-doctrine-ledger-execution.md#the-readiness-comparison-acceptance-item-4).
...
- **2026-08-31 — step 8 unit 6 shipped** (v1.4.2): the regression pass and the
  ...
  queued in [0025](0025-release-blocked-schema-drops.md) — Peter's call,
  ...
  **Regression pass** (browser, 390×844, live data, no console errors beyond the
  ...
  77.4 kg from 2026-08-17, blood with the 48 h / 21 d placeholder note, recovery
...
[010 §Grounding](0010-home-fused-reads.md#grounding) holds the numbers. Build in
...
- **Resolve first — HRV.** The gate card shows SLEEP and HRV, but `SLEEP_COLS`
  ...
  pull (`design/home-canvas/DATA.md`) lists per-night HRV and
...
- ~~Drop `show_in_home`~~ — the app side is done. The **column** is release-
  ...
  [0025-release-blocked-schema-drops.md](0025-release-blocked-schema-drops.md).
...
Answer the two open questions in [0010-home-fused-reads.md](0010-home-fused-reads.md) §6
...
- [x] **Step 8 — built.** The refined design is implemented on Home, in six
      ...
      [0010-home-fused-reads.md](0010-home-fused-reads.md) (the fused read —
      closed with this) and
      [0014-doctrine-ledger-execution.md](0014-doctrine-ledger-execution.md) (the
