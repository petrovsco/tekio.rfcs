---
title: "HR-based intensity classification for cardio & sport sessions"
authors: [Peter Petrov]
created: 2026-07-10
last_updated: 2026-09-10
status: done
status_note: "2026-09-07: the grounded classifier, the bout length and the measured sync shipped as patches (v2.0.23–v2.0.28); the two open items split out on Peter's call — [0058](0058-garmin-data-on-sport-rows.md) (Garmin data on sport rows) and [0059](0059-profile-hrmax-typed-hr-path.md) (profile HRmax and the typed-HR path), both 2.1.0."
label: feature
release: 2.1.0
---

# RFC 0005: HR-based intensity classification for cardio & sport sessions

**Note:** Narrowed 2026-09-02: inventory rows 3.7–3.9 (the cardio `rx` prose) move to [0039](0039-adaptations-read-grounding.md); this brief keeps the classifier thresholds (rows 6.1–6.5). [0031](0031-adaptations-drill-down-read.md) §3b defers its effort-plane read until this lands.
...
- 2026-09-06 (evening) — Picked up. Two science-scout runs — A: the intensity criterion for a session with Garmin data; B: the fallback with none — landed in [grounding/0005](../../grounding/005-hr-zone-intensity-classification.md#grounding) after every citation passed eutils / Crossref. Classifier rewritten on them (v2.0.23), tests moved, the dump re-run (§Result). Inventory rows 6.1–6.7, ledger D33–D36.
...
- 2026-09-07 (close) — Peter's call: the two open boxes split into [0058](0058-garmin-data-on-sport-rows.md) (Garmin data on sport rows) and [0059](0059-profile-hrmax-typed-hr-path.md) (profile HRmax and the typed-HR path), both 2.1.0; this brief moves to done/ (v2.0.30). Found on the way: all 220 `cardio_sessions` rows are Garmin-sourced, so the manual-row rules — 6.1's floor and the typed-HR path — serve no row today.
...
Two classifiers live in `src/lib/adaptations.ts`
today:
...
[docs/grounding/0005-hr-zone-intensity-classification.md](../../grounding/005-hr-zone-intensity-classification.md#grounding)
...
- **Typed avg HR on manual steady rows** (≤ ~83 % HRmax endurance, ≥ ~89–90 %
  ...
  [0059](0059-profile-hrmax-typed-hr-path.md).
...
- **Garmin data on sport rows.** A migration mirroring `cardio_sessions`'
  ...
  the function's unused parameter. **Split out 2026-09-07 →
  [0058](0058-garmin-data-on-sport-rows.md).**
- **The typed-HR path for manual steady rows.** Grounded in run B (≤ ~83 %
  ...
  **Split out 2026-09-07 → [0059](0059-profile-hrmax-typed-hr-path.md).**
...
[0058](0058-garmin-data-on-sport-rows.md); the typed-HR path for manual
