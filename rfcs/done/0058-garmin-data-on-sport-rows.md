---
title: Garmin data on sport rows
authors: [Peter Petrov]
created: 2026-09-07
last_updated: 2026-09-07
status: done
status_note: "2026-09-07 (v2.0.31–v2.0.32): migration 20260907070704 applied and mirrored, the sync writes and backfills the six columns, the three synced matches are backfilled (dry → real → dry clean: 3 backfilled, then 3 already synced), and `classifySportAdaptations` reads them through the shared core `classifyGarminIntensity`. No verdict moved — all three matches still read endurance. Split out of [0005](0005-hr-zone-intensity-classification.md) on 2026-09-07 (Peter's call) so that brief could close."
label: feature
release: 2.1.0
---

# RFC 0058: Garmin data on sport rows
...
[0041](0041-garmin-sport-activity-sync.md) stores only the duration and the
...
grounded rules of [0005](0005-hr-zone-intensity-classification.md).
...
- **A migration on the shared database.** Additive columns only — adds are
  ...
  ([0025](0025-release-blocked-schema-drops.md)) — mirroring what
