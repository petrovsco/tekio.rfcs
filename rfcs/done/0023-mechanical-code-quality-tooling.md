...
# RFC 0023: Mechanical code quality — ESLint, dead-code detection, perf budget

**Origin:** pushbacks #3 and #4 and deliverables 5 and 6 of [0009-feature-grounding.md](0009-feature-grounding.md), agreed 2026-08-26 and never started. The arguments below are carried in full — this brief is kickoff-ready on its own and you do not need to read 009 first.
...
repo, while `src/components/ui/EditModal.tsx`
and `src/components/tabs/ProgramTab.tsx`
...
1. **ESLint — done 2026-09-08 (v2.0.59).** Flat config in
   `eslint.config.js`, `npm run lint`. **Not** wired
...
2. **Dead-code detection — done 2026-09-08 (v2.0.60).** `knip`, configured in
   `knip.jsonc`, `npm run knip`. It reports and never deletes.
...
4. **`scripts/perf-budget.mjs` — done 2026-09-08 (v2.0.62).** Plus
   `scripts/perf-startup.mjs` and the committed
   `scripts/perf-baseline.json`. See
...
`src/lib/supabase.ts` now constructs that client
directly and all 30 call sites are unchanged; the one edge-function call is a
plain `fetch` in `src/lib/assistant/client.ts`,
...
- `EditModal.tsx`, 10× `react-hooks/refs`. Ten forms do `saveRef.current = save`
  ...
  [0048-simplification-candidates.md](0048-simplification-candidates.md) replaces
...
`knip.jsonc` — a bare ignore list is how a tool quietly stops
meaning anything.
...
[0048-simplification-candidates.md](0048-simplification-candidates.md), which
...
- Anything in the grounding track. Those runs stay in
  [0009-feature-grounding.md](0009-feature-grounding.md).
...
- [x] The startup timing run produces a number, and that number is written down
      ...
      `scripts/perf-baseline.json`, and
