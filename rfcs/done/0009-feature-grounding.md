---
title: Feature grounding — doctrine, science-scout, and review lenses
authors: [Peter Petrov]
created: 2026-08-25
last_updated: 2026-09-10
status: done
status_note: 2026-09-01. Deliverables shipped and acceptance all ticked. The lint/dead-code/perf half lives in 023; the back-fill remainder is tracked by its own briefs (013 for cycle/deload, 001/005 for the 3.4/3.8 cues), and item 2 (recovery weights) closed differently — RECOVERY_WEIGHTS was retired, not reweighted (see 014).
label: infra
---

# RFC 0009: Feature grounding — doctrine, science-scout, and review lenses

**Kickoff:** this file is the brief. It carries the full decision log so a fresh session can resume without the original conversation.
...
| 5 | Maintainability | existing `/simplify` + `/code-review`, plus **add ESLint + knip** and a tekio conventions file | **moved out to [0023](0023-mechanical-code-quality-tooling.md)** | tooling only |
| 6 | Performance | `npm run perf` budget script; an agent interprets the numbers | **moved out to [0023](0023-mechanical-code-quality-tooling.md)** | script, not agent |
...
[0015-ground-trigger-spec-fixes.md](0015-ground-trigger-spec-fixes.md) §13.7. Either apply
...
recovery weights in. **Resolved 2026-09-08:** 015 shipped the half that is not a
choice — exemption 1 now states that rescaling preserves a claim but never
creates one, so an `unknown` row does not clear. The fork itself was postponed to
3.0.0 as [0065](../0065-rescale-exemption-unchecked-base.md), because the weights
it argues over no longer exist to decide it against.
...
- **Home redesign + the stimulus/recovery model** — written up as
  [0010-home-fused-reads.md](0010-home-fused-reads.md) (proposed, not agreed). Recovery stops
