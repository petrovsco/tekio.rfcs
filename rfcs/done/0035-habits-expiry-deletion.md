---
title: Habits expiry — delete the shelved section, keep the muscle editor
authors: [Peter Petrov]
created: 2026-09-05
last_updated: 2026-09-05
status: done
status_note: "ran 2026-09-05, a month before the R2 date, on Peter's call: Habits is against the doctrine and the app's flow, so the wait was buying nothing. Code, tests, assistant tools and the deployed edge function are clean; the table drops are queued in 025."
label: feature
---

# RFC 0035: Habits expiry — delete the shelved section, keep the muscle editor
...
`src/components/tabs/admin/ExerciseMuscleEditor.tsx`
...
`src/components/tabs/admin/MuscleGroupEditor.tsx`
...
- The `'Habits'` entry in `DRAWER_TABS` (`src/App.tsx`),
  its `NAV_META` row in the Drawer, its `SECTION_META` row in Profile, and the
  `habits` icon in `src/components/ui/Icon.tsx`.
- The Habits row in `DEFAULTS`
  (`src/lib/db/sectionConfig.ts`). Nothing read it:
...
[0025](0025-release-blocked-schema-drops.md)). The drops are queued in 025's
