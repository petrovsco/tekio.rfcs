...
| 1.10 | `0` ×7 | `weeklyMuscleTarget` / `weeklySessionTarget` sentinels throughout [adaptations.ts](../src/constants/adaptations.ts) | *Nothing.* `0` means "this axis does not apply to this adaptation" — a stand-in for `null`, read as a flag at [lib/adaptations.ts:365](../src/lib/adaptations.ts#L365) | ? | unknown † | — |
| 1.11 | all 14, duplicated | `adaptation_targets` (DB), 9 rows — 7 live, plus dead `speed` / `skill` rows nothing reads since 2026-09-01 | Identical values to 1.2–1.5 and 1.7–1.9, and **they win** — [lib/adaptations.ts:365-366](../src/lib/adaptations.ts#L365) prefers the DB row (`targets?.[meta.key]`) over the constant | named | **grounded** | adaptation-weekly-targets |
...
| 4.11 | `(sleep + hrv) / 2` | [fusedRead.ts:441](../src/lib/fusedRead.ts#L441) | Systemic readiness is last night's Garmin sleep score blended 50/50 with the HRV sub-score (row 4.17); it degrades to sleep-only or HRV-only when one side is missing and is `null` when both are — a missing input never scores zero. The 0–100 shape and the 50/50 blend are convention: no manufacturer composite is validated as a training-decision threshold. Replaced the old card's `score / 100` sub-score, which silently switched sleep models per night | unnamed | **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) |
| 4.12 | `PUSH_THRESHOLD = 33` | [app.ts:85](../src/constants/app.ts#L85), applied at [fusedRead.ts:510](../src/lib/fusedRead.ts#L510) | Readiness below 33 flips the verdict to Hold (was the old card's 80 / 50 green–amber–red bands). **This is the app's answer to "am I recovered enough to push today?"** — and Hold means modify, not rest (D7) | named | **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) — any fixed cutoff on a 0–100 composite is convention; grounded method is baseline-relative HRV (D8) |
...
| 4.14 | `80` °C / `10` °C | [RecoverySheet.tsx:42](../src/components/tabs/home/RecoverySheet.tsx#L42), [:50](../src/components/tabs/home/RecoverySheet.tsx#L50) (quick-log defaults); [EditModal.tsx:740](../src/components/ui/EditModal.tsx#L740), [:751](../src/components/ui/EditModal.tsx#L751) (placeholders) | Default temperatures for a sauna / cold session. Stored, never scored | ? | unknown † | — |
...
| 6.12 | `HR_MAX_REPLICATION_BPM = 3` | [lib/hrMax.ts:10](../src/lib/hrMax.ts#L10) | **New row 2026-09-07** (059): a session max HR counts as the observed HRmax only when a second session comes within 3 bpm of it — a replicated peak, never the single highest reading, because a wrist-optical spike can sit 20 bpm above anything a second session reaches (Navalta 2020; Peter's 214) and Garmin's auto-detect ratchets to it. The same 3 bpm is the margin by which a synced peak overrules a typed override. A strap's precision plus day-to-day noise; no study tests a replication or percentile rule (D37) | named | convention (D37) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
...
| 10.4 | ~~`56 * 86400000`~~ | — | **Fixed 2026-08-31.** The literal went with OverviewTab when the fused Home shipped (018 unit 4); `donationStatus` reads `DONATION_ELIGIBILITY_DAYS` ([fusedRead.ts:466](../src/lib/fusedRead.ts#L466)), so 10.2 is the one copy | — | — | — |
