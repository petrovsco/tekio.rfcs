...
| D34 | **Session-goal order, and the Z5 dose replaces the zone share** — structure first (`format = 'intervals'` is never endurance; bout ≤ `ANAEROBIC_BOUT_MAX_S = 120` s → anaerobic, longer → VO₂max when the row carries `bout_seconds` — column and form field since 2026-09-07, and the sync fills it from Garmin's `INTERVAL_ACTIVE` split the same day; without it Garmin's anaerobic > aerobic TE is the vendor tie-break), then `VO2MAX_Z5_MIN = 8` minutes at ≥ 90 % HRmax (range 5–10; HR lag inside 4-min bouts), then the label only when zones are absent. "Z4+Z5 > Z1+Z2" had no source and Garmin Z4 straddles the threshold band. **Fork 1: shipped as (a) on 2026-09-06, (b) since 2026-09-07 on Peter's call** — TEMPO / LACTATE_THRESHOLD runs fall to the aerobic floor and credit endurance: the classifier says what a session trained, not whether it was the polarized way to train it, and under (a) a third of the running history sat in an uncounted eighth bucket. 039 S9's prescription (Zone 2) is the default shape, not the boundary | [Seiler & Kjerland 2006](https://pubmed.ncbi.nlm.nih.gov/16430681/); [Sylta 2014](https://pubmed.ncbi.nlm.nih.gov/24408353/); [Buchheit & Laursen 2013](https://pubmed.ncbi.nlm.nih.gov/23539308/); [Seiler 2013](https://pubmed.ncbi.nlm.nih.gov/21812820/); [Seiler & Tønnessen 2009](https://www.sportsci.org/2009/ss.htm) | [005 A](grounding/005-hr-zone-intensity-classification.md#grounding) |
...
| D37 | **HRmax is a replicated observed peak, not a formula** — `observedHrMax`: the highest session max HR inside a rolling `HR_MAX_WINDOW_MONTHS = 24` window that a second session comes within `HR_MAX_REPLICATION_BPM = 3` bpm of, land modalities only (a swimmer's ceiling is 6–7 bpm lower); a typed override stands in for a chest-strap test and loses to a synced peak more than 3 bpm above it. Every age formula misses an individual by ±9–11 bpm RMSE and none improves with training status; trained athletes' field peaks sit 5–13 bpm above the formulas and the lab; a wrist-optical spike (Peter's 214) is what Garmin's one-way auto-detect ratchets to, and 220 − age has no research origin. No formula fallback and no birth year: a ±10 bpm denominator cannot separate the 83 % and 89 % cuts, so with no peak and no override the path is off and the row reads as before. Window and tolerance are conventions inside 12–36 months and a strap's precision (2026-09-07). The no-formula, no-birth-year half, and the silent overrule of a typed override, were reversed the same evening — D39 | [Kasiak 2023](https://pubmed.ncbi.nlm.nih.gov/37109218/); [Martin 2025](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0335842); [Ausland 2026](https://pubmed.ncbi.nlm.nih.gov/42088591/); [Semin 2008](https://pubmed.ncbi.nlm.nih.gov/24149950/); [Navalta 2020](https://pubmed.ncbi.nlm.nih.gov/32866216/); [Olstad 2019](https://pubmed.ncbi.nlm.nih.gov/31726693/); [Zavorsky 2000](https://pubmed.ncbi.nlm.nih.gov/10688280/) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
| D38 | **The typed HR picks the bucket, the floors decide the credit; sport rows never promote** — on a steady/unstated cardio row with no Garmin data, `typedHrBand` in whole percent: ≥ `HR_VO2MAX_MIN_PCT = 89` is VO₂max at the `VO2MAX_Z5_MIN` dose (a steady row's minutes at ≥ 90 % are its duration), anything below is endurance at `ENDURANCE_FLOOR_MIN`; `HR_ENDURANCE_MAX_PCT = 83` is the edge of the 84–88 % threshold band, a label (057 B) that moves no credit; no lower cut, since 005 B set none. A hand-logged match with a typed HR stays endurance — HR over-reads tennis by ~7 % of HRmax at equal V̇O₂, and Z5 = 0 on every synced match (2026-09-07) | [Stöggl & Sperlich 2015](https://pmc.ncbi.nlm.nih.gov/articles/PMC4621419/) (Tønnessen bands); [Buchheit & Laursen 2013](https://pubmed.ncbi.nlm.nih.gov/23539308/); [Helgerud 2007](https://pubmed.ncbi.nlm.nih.gov/17414804/); [Jamnick 2020](https://pubmed.ncbi.nlm.nih.gov/32729096/); [Ferrauti 2001](https://pubmed.ncbi.nlm.nih.gov/11513317/) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) · [005 B](grounding/005-hr-zone-intensity-classification.md#grounding) |
| D39 | **HRmax for any user: the age estimate is the default, the tracker peak is offered and accepted, a sync never overwrites a stored number** — reverses the no-formula, no-birth-year half of D37 (Peter, 2026-09-07: "this is built only for me"). A derivation that needs 24 months of synced rows produces nothing for a user without a tracker, who is the only user the typed-HR path serves — tracker rows carry Training Effect and zones and never reach it. So: `formulaHrMax` (Tanaka, 208 − 0.7 × age) from `user_profiles.birth_date` when nothing is stored, shown as an estimate with its ±10 bpm band; `hrMaxProposal` offers `observedHrMax` on the Profile when nothing is stored or the peak sits more than 3 bpm above it, offered again every time (fork C); one stored number with `hr_max_source` typed / tracker, last write wins (fork B); a hint on the cardio form when a typed HR has no denominator (fork A). The ±9–11 bpm objection stands recorded in 060 and was overruled: a fuzzy default beats no default, and the tracker corrects it (2026-09-07) | [Tanaka 2001](https://pubmed.ncbi.nlm.nih.gov/11153730/); [Kasiak 2023](https://pubmed.ncbi.nlm.nih.gov/37109218/); [Martin 2025](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0335842) | [060](roadmap/done/060-hrmax-any-user-birth-date-default.md) · [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
...
| 1.10 | `0` ×7 | `weeklyMuscleTarget` / `weeklySessionTarget` sentinels throughout [adaptations.ts](../src/constants/adaptations.ts) | *Nothing.* `0` means "this axis does not apply to this adaptation" — a stand-in for `null`, read as a flag at [lib/adaptations.ts:381](../src/lib/adaptations.ts#L381). **Definitional** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): a flag carries no dose to test. §13.8 left the *gating* question open, not this one — the real fix is a proper `null`, which is a code change | ? | **n/a — definitional** | — |
...
| 4.12 | `PUSH_THRESHOLD = 33` | [app.ts:95](../src/constants/app.ts#L95), applied at [fusedRead.ts:492](../src/lib/fusedRead.ts#L492) | Readiness below 33 flips the verdict to Hold (was the old card's 80 / 50 green–amber–red bands). **This is the app's answer to "am I recovered enough to push today?"** — and Hold means modify, not rest (D7) | named | **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) — any fixed cutoff on a 0–100 composite is convention; grounded method is baseline-relative HRV (D8) |
...
| 4.14 | `80` °C / `10` °C | [RecoverySheet.tsx:36](../src/components/tabs/home/RecoverySheet.tsx#L36), [:44](../src/components/tabs/home/RecoverySheet.tsx#L44) (quick-log defaults); [EditModal.tsx:668](../src/components/ui/EditModal.tsx#L668), [:679](../src/components/ui/EditModal.tsx#L679) (placeholders) | Default temperatures for a sauna / cold session. Stored, never scored — but the quick-log *writes* 80 / 10 into the row, so they become data rather than a hint. **Left `unknown`** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): an assumed fact about a real session, not a unit or a guard, and heat / cold exposure has a dose literature of its own. Arguable, so §13.2's "keep it small" rule keeps it here | ? | unknown † | — |
| 4.15 | `RECOVER_DAYS = 2` | [app.ts:89](../src/constants/app.ts#L89) | 48 h since a muscle's last hard set is the local recovery flag — the floor of the 48–72 h band, dose-blind by design; dose modulation is the named upgrade path (D6). Sat in §12 as not-yet-built until 2026-09-05; it shipped with the fused Home (018 unit 4) | named | **grounded** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) |
| 4.16 | `48` h / `21` d | [app.ts:135](../src/constants/app.ts#L135), read at [fusedRead.ts:454-455](../src/lib/fusedRead.ts#L454) | `DONATION_SUPPRESSION`: a full-blood donation gates a Hold for 48 h and dims the two aerobic reads for 21 d; strength and anaerobic never dim; plasma gets nothing (D11). Shipped with the fused Home, indexed here since 2026-09-05 | named | **grounded** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) |
| 4.17 | `7` d rolling / `60` d baseline / `50 + 50 × z` | [fusedRead.ts:364-365](../src/lib/fusedRead.ts#L364), [:412](../src/lib/fusedRead.ts#L412) | The HRV sub-score is baseline-relative: the 7-day rolling mean of overnight HRV placed against a 60-day baseline in SD units — 50 at baseline, 0 one SD below, 100 one SD above. A rolling window against the individual's own baseline is the method every trial used (Vesterinen 2016; Buchheit 2014); the 60-day baseline and the SD-to-0–100 scale are conventions (practitioners say 21–30 d). `MIN_HRV_BASELINE_SAMPLES = 7` and `HRV_SD_FLOOR = 0.05` are guards, not claims | unnamed | method **grounded**, scale **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) |
...
| # | Value | Where | Claim | Step 0 | State | Grounding brief |
|---|---|---|---|---|---|---|
| 5.1 | `CYCLE = 6` | [app.ts:13](../src/constants/app.ts#L13) | A training block is 6 weeks | named | unknown | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
| 5.2 | ~~`CYCLE = 6`~~ | — | **Fixed 2026-08-26** — `utils.ts` imports `CYCLE` from `constants/app` | — | — | — |
| 5.3 | `DELOAD_WEEK = CYCLE` | [app.ts:24](../src/constants/app.ts#L24), used at [utils.ts:108](../src/lib/utils.ts#L108), [:115](../src/lib/utils.ts#L115) | **Week 6 is the deload week** — deload placement. Named as of 2026-08-26; still ungrounded | named | unknown | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
| 5.4 | `DELOAD_REP_FACTOR = 0.7` | [app.ts:31](../src/constants/app.ts#L31), applied by `deloadSets` at [utils.ts:123](../src/lib/utils.ts#L123) | Deload = 70% of last reps, load unchanged. **Fixed 2026-08-26** — was three implementations, two of which disagreed | named | unknown | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
...
| 5.7 | `cycle_length_weeks: CYCLE` | [db/program.ts:229](../src/lib/db/program.ts#L229) + `programs` column default `6` | **Fixed 2026-08-26** — was hardcoded `6`; now derives from 5.1. Still write-only. **Stays `unknown`** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): §13.2 names this exact boundary — "a block is 6 weeks" is a dose claim, not a definition. The run that settles it lands on 5.1 | named | unknown † | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
| 5.8 | `deload_week: DELOAD_WEEK` | [db/program.ts:230](../src/lib/db/program.ts#L230) | **Fixed 2026-08-26** — was hardcoded `6`; now derives from 5.3. Still write-only. **Stays `unknown`** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): deload *placement* is the dose claim §13.2 draws the line against; the run lands on 5.3 | named | unknown † | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
| 5.9 | `factor: DELOAD_REP_FACTOR` | [db/program.ts:231](../src/lib/db/program.ts#L231) + `programs.deload_strategy` column default | **Fixed 2026-08-26** — the write now derives from 5.4. The **jsonb column default** still carries a literal `0.7`, and nothing reads either | unnamed | unknown | [cycle-deload-grounding](roadmap/013-cycle-deload-grounding.md) |
...
| 6.5 | `SPORT_DEFAULT_ADAPTATION` = `'endurance'` | [lib/adaptations.ts:131](../src/lib/adaptations.ts#L131) | **Moved 2026-09-06** from `vo2max`: a tennis match is ~70–75 % HRmax, ~52 % V̇O₂max, ~77 % of playing time below VT1 and ~3 % above VT2 with 5–10 s rallies — aerobic-base work, timed or not, singles and doubles alike (D36). Since 058 (2026-09-07) only a row with no Garmin data reads this — a synced match carries the same TE and zones as a cardio row and goes through the shared core (`classifyGarminIntensity`), so a match with 8 min in Z5 is VO₂max and one below the aerobic floor credits nothing | named | convention (VO₂max default not supported — D36) | [005 B](grounding/005-hr-zone-intensity-classification.md#grounding) |
| 6.6 | ~~`THRESHOLD_LABELS`~~ (TEMPO, THRESHOLD) | — | **Retired 2026-09-07** (fork 1b, Peter's call). Lived one day: added 2026-09-06 so Garmin's TEMPO / LACTATE_THRESHOLD label (threshold work, LT1–LT2) credited nothing — the polarized reading. Now these runs fall to the aerobic floor (6.3) and credit endurance, the adaptation they train by a harder route than Zone 2; the old regex had sent them to VO₂max (D34) | — | — | [005 A](grounding/005-hr-zone-intensity-classification.md#grounding) |
| 6.7 | `VO2MAX_LABELS` (VO2, ANAEROBIC, SPRINT, SPEED) | [lib/adaptations.ts:63](../src/lib/adaptations.ts#L63) | **New row 2026-09-06**: Garmin's VO₂max-family label stands in for the Z5 dose only when the row carries no zones — the label is a heuristic over the same TE + time-in-zone inputs (Firstbeat patent US 11771355 B2), not a measurement (D34) | named | convention (vendor heuristic — D34) | [005 A](grounding/005-hr-zone-intensity-classification.md#grounding) |
| 6.8 | `ANAEROBIC_BOUT_MAX_S = 120` | [lib/adaptations.ts:60](../src/lib/adaptations.ts#L60) | **New row 2026-09-07**: on an `intervals` row the work-bout length (`cardio_sessions.bout_seconds` — typed in the form when the format is intervals, or measured by the watch's timer and filled by the sync from Garmin's `INTERVAL_ACTIVE` split since 2026-09-07) decides first — ≤ 120 s is anaerobic-capacity work, longer is VO₂max. The only duration physiology is per effort: aerobic and anaerobic contributions are equal at ≈ 75 s and longer efforts are aerobically dominated (Gastin 2001); Tekiō's own protocols sit either side (3.7: 20 s–2 min all-out; 3.8: 3–8 min intervals; Seiler & Tønnessen 2009: VO₂max bouts ~1–8 min). The Z5 dose and the TE tie-break are the fallbacks when the row has no bout (D34) | named | grounded (D34) | [005 A](grounding/005-hr-zone-intensity-classification.md#grounding) |
| 6.9 | `HR_VO2MAX_MIN_PCT = 89` | [lib/adaptations.ts:493](../src/lib/adaptations.ts#L493) | **New row 2026-09-07** (059): on a steady/unstated cardio row with no Garmin data, a typed average HR at ≥ 89 % of the profile HRmax is VO₂max work — Tønnessen's Z4 floor, Garmin's Z5 line and Buchheit & Laursen's ≥ 90 % criterion coincide — credited at the `VO2MAX_Z5_MIN` dose, since a steady row's minutes at ≥ 90 % are its duration. Whole-percent bands, a ±5 % band not a line (Jamnick 2020) (D38) | named | grounded (partially supported — the cut is 005 B's, the denominator D37's) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
| 6.10 | `HR_ENDURANCE_MAX_PCT = 83` | [lib/adaptations.ts:490](../src/lib/adaptations.ts#L490) | **New row 2026-09-07** (059): at or below 83 % HRmax a typed average HR is endurance (Tønnessen Z2 74–83 %); 84–88 % is the threshold band, a *label* (057 B) that moves no credit, so this cut changes no bucket on its own — everything below 89 % is endurance at the `ENDURANCE_FLOOR_MIN` floor. No lower cut: 005 B set none (D38) | named | grounded (partially supported — D38) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
| 6.11 | `HR_MAX_WINDOW_MONTHS = 24` | [lib/hrMax.ts:7](../src/lib/hrMax.ts#L7) | **New row 2026-09-07** (059): the observed HRmax is read over a rolling 24-month window of synced rows. Age moves HRmax ~0.7 bpm/yr (Gellish 2007; under 1.5 bpm inside the window), training status 3–7 % (Zavorsky 2000); a shorter window mistakes fewer all-out efforts for drift (Peter's 12-month read is 191, his 24-month read 196). A convention inside 12–36 months (D37) | named | convention (D37) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
| 6.12 | `HR_MAX_REPLICATION_BPM = 3` | [lib/hrMax.ts:10](../src/lib/hrMax.ts#L10) | **New row 2026-09-07** (059): a session max HR counts as the observed HRmax only when a second session comes within 3 bpm of it — a replicated peak, never the single highest reading, because a wrist-optical spike can sit 20 bpm above anything a second session reaches (Navalta 2020; Peter's 214) and Garmin's auto-detect ratchets to it. The same 3 bpm is the margin above the stored number at which the tracker's peak is offered again (D39). A strap's precision plus day-to-day noise; no study tests a replication or percentile rule (D37) | named | convention (D37) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |
| 6.13 | `HR_MAX_FORMULA_INTERCEPT = 208` | [lib/hrMax.ts:65](../src/lib/hrMax.ts#L65) | **New row 2026-09-07** (060): the default HRmax when the user has set no number is the Tanaka age formula, 208 − 0.7 × age, from the profile's birth date, shown on the Profile as an *estimate* with its ±10 bpm band. Lowest RMSE of the age formulas in athletes (Kasiak 2023: 9.2 bpm) and no better in adults at large (Martin 2025: 9.2–10.9 bpm); never 220 − age, which has no research origin. Never stored: a typed number or an accepted tracker peak replaces it (D39) | named | grounded (partially supported — a group-mean fit, ±9–11 bpm for one person; the scout's own fallback in 059) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) · [060](roadmap/done/060-hrmax-any-user-birth-date-default.md) |
| 6.14 | `HR_MAX_FORMULA_SLOPE = 0.7` | [lib/hrMax.ts:68](../src/lib/hrMax.ts#L68) | **New row 2026-09-07** (060): bpm of HRmax lost per year of age in the same formula — Tanaka's cross-sectional slope, matched within-person by Gellish 2007 (0.7 bpm/yr) and by the 8.5-year fall in master runners (Hawkins 2001) (D39) | named | grounded (partially supported — D39) | [059 §Grounding](roadmap/done/059-profile-hrmax-typed-hr-path.md#grounding) |

## 7. Muscle & volume accounting

| # | Value | Where | Claim | Step 0 | State | Grounding brief |
|---|---|---|---|---|---|---|
| 7.1 | `{1: 1, 2: 0.5, 3: 0}` | [utils.ts:475](../src/lib/utils.ts#L475) | A level-2 muscle receives half a set's stimulus; level 3 receives none — after the 042 audit it holds stabilisers and bystanders only. **Every muscle-coverage number and the whole BodyMap is denominated in this** | unnamed | grounded (level 2 supported; level 3 grounded at 0, 2026-09-03) | [039 S1](grounding/039-adaptations-read.md#grounding) + [done/042](roadmap/done/042-level-3-link-audit.md) |
| 7.2 | `level === 1 → primary` | [db/muscles.ts:79](../src/lib/db/muscles.ts#L79) | Level 1 is a primary mover; 2 and 3 are both "secondary" — collapses 7.1's three tiers into two on write. **Stays `unknown`** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): a classification, and §13.4 now gates those on the same terms as a coefficient. Write-only, though — nothing reads `role` back — so no run of its own | unnamed | unknown † | home-fused-reads |
...
| 9.2 | `min 5 / max 10` | [ExPlan.tsx:77](../src/components/tabs/weights/ExPlan.tsx#L77) | The defensible weekly-progression band is 5–10% | unnamed | unknown | **(no brief)** |
...
| 9.4 | `r05` — round to `0.5` | [utils.ts:93](../src/lib/utils.ts#L93) | Plate granularity. **Definitional** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): rounding a computed weight to the nearest 0.5 kg fixes the unit the app prints — the canonical case in §13.2's own text | no | **n/a — definitional** | — |
...
| # | Value | Where | Claim | Step 0 | State | Grounding brief |
|---|---|---|---|---|---|---|
| 10.1 | `2500` ml | [app.ts:33](../src/constants/app.ts#L33) | Daily hydration target | unnamed | unknown | home-fused-reads **(due — folding)** |
| 10.2 | `56` days | [app.ts:78](../src/constants/app.ts#L78) | Full-blood donation interval | unnamed | **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) — service rule, not physiology; calendar only |
| 10.3 | `14` days | [app.ts:79](../src/constants/app.ts#L79) | Plasma donation interval | unnamed | **convention** | [010 §Grounding](roadmap/done/010-home-fused-reads.md#grounding) — service rule, not physiology; calendar only |
| 10.4 | ~~`56 * 86400000`~~ | — | **Fixed 2026-08-31.** The literal went with OverviewTab when the fused Home shipped (018 unit 4); `donationStatus` reads `DONATION_ELIGIBILITY_DAYS` ([fusedRead.ts:448](../src/lib/fusedRead.ts#L448)), so 10.2 is the one copy | — | — | — |
| 10.5 | `[100, 250, 500]` | [FoldSheet.tsx:42](../src/components/tabs/home/FoldSheet.tsx#L42) | Quick-add water increments — UI affordance. **Definitional** (2026-09-08, [066](roadmap/done/066-inventory-definitional-rows.md)): three cup and bottle sizes to tap, chosen so any total is reachable in a few taps. The hydration dose is 10.1, which stays `unknown` | no | **n/a — definitional** | — |
...
| `revealed < 8` / `revealedEx < 8` | [SupersetLogger.tsx:122](../src/components/tabs/weights/SupersetLogger.tsx#L122), [MobilityTab.tsx:167](../src/components/tabs/MobilityTab.tsx#L167) | Progressive disclosure |
